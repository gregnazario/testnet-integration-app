  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const [coinBalance, faBalance] = await Promise.all([
          getCoinBalance({
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex gap-4">
        <div className="flex items-center gap-2">
          <input type="radio" id="coin" name="type" checked={!isFa} onChange={() => setIsFa(false)} />
          <label htmlFor="coin">Coin</label>
        </div>
        <div className="flex items-center gap-2">
          <input type="radio" id="fa" name="type" checked={isFa} onChange={() => setIsFa(true)} />
